...
void addTapDevice(const wchar_t *deviceId)
{
	...
	auto status = SetupDiCreateDeviceInfo(
		deviceInfoSet,
	...
	status = SetupDiSetDeviceRegistryProperty(
		...
		&devInfoData,
		SPDRP_HARDWAREID,
		reinterpret_cast<const BYTE*>(hwidList[0]),
		sizeof(hwidList)
	);
	...
	PluginLog(L"SetupDiCallClassInstaller() succeeded");
}
...
void recreateTapDevice(HDEVINFO devInfo, PSP_DEVINFO_DATA devInfoData)
{
	...
	status = SetupDiSetClassInstallParams(devInfo, devInfoData, &rmdParams.ClassInstallHeader, sizeof(rmdParams));
	THROW_GLE_IF(FALSE, status, "SetupDiSetClassInstallParams()");
...
void updateTapDriver()
{
	...
	PluginLog(L"Attempting to install new driver");
	...
	DWORD installFlags = 0; // INSTALLFLAG_NONINTERACTIVE;
...
ATTEMPT_UPDATE:
	...
	if (result != TRUE)
	{
		...
		{
			PluginLog(L"Driver update failed. Attempting forced install.");
			installFlags |= INSTALLFLAG_FORCE;
...
//static
void Context::CreateTap()
{
	// TODO: take care of naming
	// TODO: return value?
	addTapDevice(nullptr);
}

//static
void Context::UpdateDriver()
{
	// TODO: return value?
	updateTapDriver();
}
